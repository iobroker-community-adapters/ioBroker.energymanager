...
This adapter (`ioBroker.energymanager`) reads energy management data from kiwigrid-powered devices via their local REST API. Confirmed compatible devices include E.ON Aura and Solarwatt MyReserve; other kiwigrid-based devices (innogy, enviam) may also work.

- **Adapter Name:** energymanager
- **Primary Function:** Poll kiwigrid REST API at `http://{managerAddress}/rest/kiwigrid/wizard/devices` to read device tag values and expose them as ioBroker states
- **Key Dependencies:** `request` (HTTP client), `@iobroker/adapter-core`
- **Connection Type:** Local network (polls the device on the LAN; no cloud dependency)
- **Data Source:** Periodic polling (configurable interval via `managerIntervall`)
- **Configuration Requirements:** `managerType` (device type), `managerAddress` (IP/hostname), `managerIntervall` (poll interval in seconds), `managerPassword` (for reboot command), `managerRounding` (W/kW or Wh/kWh), `managerStateExpireTimeout` (state expiry)
- **State Structure:** States are grouped by device class (e.g., `Location_(name)`, `PVPlant_(name)`, `Inverter_(name)`) with tag names as state IDs
- **Multilingual Support:** Device tag names are translated using `lang/i18n/{lang}/translations.json` based on ioBroker system language
...
- **Note:** This adapter currently uses the `request` library (deprecated). New features should prefer `fetch`; migration to `fetch` is recommended when refactoring
...
**Kiwigrid REST API Pattern:**
```javascript
// Poll device values
const response = await fetch(`http://${adapter.config.managerAddress}/rest/kiwigrid/wizard/devices`);
if (!response.ok) {
    adapter.log.error(`API error: ${response.status} ${response.statusText}`);
    return;
}
const content = await response.json();
// Process content.result.items[]

// Reboot command (POST with password)
const loginResponse = await fetch(`http://${adapter.config.managerAddress}/rest/login`, {
    method: 'POST',
    body: JSON.stringify({ password: adapter.config.managerPassword }),
});
await fetch(`http://${adapter.config.managerAddress}/rest/reboot`, { method: 'POST' });
```
...
## Adapter-Specific Development Notes

### Kiwigrid REST API Integration

- **Base URL:** `http://{managerAddress}/rest`
- **Devices endpoint:** `/kiwigrid/wizard/devices` — returns all device tag values
- **Login endpoint:** `/login` — POST with `{ password: "..." }` for authenticated operations
- **Reboot endpoint:** `/reboot` — POST after login to reboot the hardware manager
- **Authentication:** Cookie-based session (`request.jar()`) for reboot flow; device polling is unauthenticated
- **Response format:** `content.result.items[]` with `tagValues` and `deviceModel` arrays
- **No cloud dependency:** All communication is local network only

### State Grouping and Naming

- States are grouped by translated device class name and device `IdName`
- Device class determines state group: `Location`, `PVPlant`, `Inverter`, `PowerMeter` get `_(IdName)` suffix
- Other device classes use just the translated class name
- Tag values with `null` or empty group are silently skipped
- `IdName` values are sanitized: spaces → `_`, dots removed, `ß` → `SS`

### Unit and Role Assignment

- Energy tags (`Work*`): `Wh` or `kWh` (based on `managerRounding` config)
- Power tags (`Power*`): `W` or `kW` (based on `managerRounding` config)
- Temperature tags: `°C` with role `value.temperature`
- Battery tags (`StateOfCharge*`): `%` with role `value.battery`
- Date tags: converted to millisecond timestamps, role `value.time`
- Price tags: `ct/kWh`
- Voltage tags: `V`

### State Expiry Behavior

- `managerStateExpireTimeout = '-1'`: States never expire
- `managerStateExpireTimeout = ''` (default): States expire after `managerIntervall * 2` seconds
- Any other value: States expire after the configured number of seconds
- This ensures stale data is flagged if polling fails

### Translation System

- Tag names and device class names are translated using `lang/i18n/{lang}/translations.json`
- System language is read from `system.config` on startup
- Falls back to English if language is not set
- Translation files are separate from admin UI translations in `admin/i18n/`
